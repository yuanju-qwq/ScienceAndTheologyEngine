...
// P2.E: loads config/engine.json before init; falls back to defaults if
// the file is missing so the engine always runs out-of-the-box.
...
int main(int argc, char* argv[]) {
    ...
    // Locate the engine root BEFORE loading config, so config/engine.json
    ...
    const std::string config_path =
        snt::core::path_utils::resolve("config/engine.json");
